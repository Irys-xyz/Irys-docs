...
Data uploaded to Irys is given a millisecond-accurate timestamp, attributes and authorship details before being passed to Arweave for permanent storage. This information is used to create a signed receipt that be trustlessly [verified](./irys-sdk/api/utils-verifyReceipt).
...
In addition to using the [query package](/querying/query-package) to search Irys and Arweave, you can also search directly from the Irys SDK. The following shows how to search for all transactions posted to [Irys mainnet](/overview/networks), paid for with Matic, and uploaded during a three day period.

For more details on all search functions, see [query package documentation](/querying/query-package).
