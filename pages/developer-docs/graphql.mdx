...
<Callout type="info" emoji="ℹ️">
	...
	tutorial](/hands-on/tutorials/graphql-explorer) where you’ll learn how to setup a
	project, install the Apollo GraphQL libraries and then build a tool to search Arweave
	for recently uploaded image files.
</Callout>
...
<img
	...
	src="/img/graphql/BundlrGQL-Anatomy.png"
/>
...
| `tags`     | An array of tag name / value pairs passed as JSON objects. Values are ANDed together, matching results will include transactions matching all of the supplied tags.                                        |
...
<img
	className="border border-[#FEF4EE] rounded mt-5 w-5/6"
	src="/img/graphql/BundlrGQL-Sandbox.png"
/>
...
query getByIds {
	transactions(
		ids: [
			"--52WQHJIJod_rni8pkl1Vxt9MFGoXZAm8SC7ex6C1o"
			"--52THRWpX_RJzGcNXmtQ2DSP37d1e1VQ4YmvbY5ZXo"
		]
	) {
		edges {
			node {
...
query getByOwner {
	transactions(
		owners: [
			"0xBcb812C6e26F4F0F78Bd7B6222461FF24F2942AE"
...
<img
	className="border border-[#FEF4EE] rounded mt-5 w-5/6"
	src="/img/graphql/ArweaveGQL-Anatomy.png"
/>
...
<img
	className="border border-[#FEF4EE] rounded mt-5 w-5/6"
	src="/img/graphql/ArweaveGQL-Sandbox.png"
/>
...
query getByIds {
	transactions(
		ids: [
			"1t-Z0oDxQLvACpLXUwwfbplw7vHialWH8mkNAvzTkDw"
			"1w6PBHQ5e0hMJWsfRCJr66JvQAcyws0k4D97kG4GVm8"
		]
	) {
		edges {
			node {
