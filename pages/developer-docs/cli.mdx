...
## Private Keys for Funding and Signing
...
### Using Private Keys with Ethereum and Solana
...
Use the `price` command, followed by a number of bytes to get the cost to upload that number of bytes. You must also provide a currency (`-c`) and a node URL (`-h`)
