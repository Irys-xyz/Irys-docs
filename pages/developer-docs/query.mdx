...
<div className="mt-7">
	...
	The Irys Query package simplifies querying Irys and Arweave.
</div>
...
Use the `currency()` function to search based on the token name used to pay for the upload. Any of [these values](https://docs.irys.xyz/overview/supported-tokens) are acceptable.

```js
const results = await myQuery.search("irys:transactions").currency("solana");
```
...
```js
const results = await myQuery.search("irys:transactions").currency("ethereum").sort("ASC");
```

or descending (`DESC`) order.

```js
const results = await myQuery.search("irys:transactions").currency("matic").sort("DESC");
```
...
```js
// Create the stream
const stream = await myQuery.search("irys:transactions").currency("solana").stream();
