...
The Irys query package features:
...
To retrieve the 20 latest transactions with the tag `Content-Type` set to `image/png` on Irys, use:
...
The `Query` class links to a GraphQL endpoint for query execution, defaulting to `https://node1.irys.xyz/graphql` for Irys and `https://arweave.net/graphql` for Arweave.

When querying Irys, you must query the same node you uploaded to. To change node, pass any of the following to the `Query` object constructor:
...
Use the `from()` function to search by wallet addresses used when signing and paying for the upload. Addresses from any of [Irys' supported chains](https://docs.irys.xyz/overview/supported-tokens) are accepted.
...
Use the `token()` function to search based on the token name used to pay for the upload. Any of [these values](https://docs.irys.xyz/overview/supported-tokens) are acceptable.
