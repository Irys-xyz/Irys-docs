...
# `irys.provenance.uploadFolder()`

Uploads a folder for [permanent storage](/learn/permanent-storage) and [strong provenance](/learn/strong-provenance).
...
-   `response`: An array of [receipts](/learn/receipts) in the form of a JSON object with the following values:
...
Files uploaded via `irys.provenance.uploadFolder(folderToUpload)` can be retrieved in one of two ways.
