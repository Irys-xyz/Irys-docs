...
The code to create a `WebIrys` object differs depending on if you're using an [external wallet](https://docs.privy.io/guide/frontend/wallets/external) vs an [embedded one](https://docs.privy.io/guide/frontend/embedded/overview). When using external wallets, you pass the Ethers provider returned from the `Wallet` object, when using embedded wallets you pass the `sendTransaction` Privy hook.
