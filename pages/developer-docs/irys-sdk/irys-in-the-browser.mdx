...
const getIrys = async (): Promise<WebIrys> => {
	const wallet = { name: "solana", provider: wallet };
...
Initialize the Privy provider with your app ID:
...
Then define a component called `PrivyIrys` and write code to login with Privy and create a new `WebIrys` object:
...
// Get a reference to the user's wallet
...
const getWebIrys = async () => {
	...
	const client = await w?.getEthersProvider();
	if (!client) throw new Error(`Cannot find privy wallet`);

	const irys = new WebIrys({ url, token, wallet: { name: "privy", client } });
	await webIrys.ready();
};
```
