...
-   [Encrypted Uploader](./provenance-toolkit/encrypted-uploader): Encrypt files before uploading.
-   [Gasless Uploader](./provenance-toolkit/gassless-uploader): Pay for user uploads server-side.
-   [Solana NFT Minter](./provenance-toolkit/solana-nft-minter): Upload an image and mint on Solana as an NFT.
-   [Transaction Feed](./provenance-toolkit/transaction-feed): Query Irys transactions.
...
-   `getIrys.ts`: Instantiates an Irys object using the parameters in `.env.local`. Currently designed to work with the Ethers 5 provider. Yo use a different provider, modify code here.
