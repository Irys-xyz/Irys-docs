...
If you don't choose to show the receipt, you can retrieve it later using the upload transaction id and the function [`irys.utils.getReceipt()`](/developer-docs/sdk/api/utils-getReceipt).
...
When showing a receipt, it is first retrieved using [`irys.utils.getReceipt()`](/developer-docs/sdk/api/utils-getReceipt`) and then set to the `receipt` state variable.
