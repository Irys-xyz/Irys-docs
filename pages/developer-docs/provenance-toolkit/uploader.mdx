...
The component uses [lazy funding](/developer-docs/sdk/api/fund#lazy-funding), only funding when the currently funded balance is not sufficient to cover the cost of this upload. Once uploaded, file URLs are stored in the `fileUrls` state variable.

When uploading a single file, the URL generated is in the form `https://arweave.net/[transaction-id]`. When uploading multiple files, they are grouped together and uploaded as a group along with a manifest. The URL generated is in the form `https://arweave.net/[manifest-id]/[file-name]`.

## Image & receipt preview

<img
	className="border border-[#FEF4EE] rounded-xl mt-5 md:w-3/6 w-full"
	src="/img/provenance-toolkit/uploader-image-preview.png"
/>
...
If you don't choose to show the receipt, you can always retrieve it later using the upload transaction id and the function [`bundlr.utils.getReceipt()`](/developer-docs/sdk/api/utils-getReceipt`).

## Customizing the UI

In default mode, the `Uploader` component shows both the image preview and receipt preview buttons, which can be optionally hidden by passing config parameters to the component.
...
To download data uploded to Arweave, [use a gateway](/learn/gateways). The Uploader component is currently configured to use the Arweave gateway, to change this modify this variable.

```ts
const GATEWAY_BASE = "https://arweave.net/";
```
...
When showing a receipt, it is first retrieved using [`bundlr.utils.getReceipt()`](/developer-docs/sdk/api/utils-getReceipt`) and then set to the `receipt` state variable.
