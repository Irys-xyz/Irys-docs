...
| Hide the file preview | `<ProgressBarUploader showPreview = {false } />` |

If `showPreview` parameter is omitted, the component defaults to showing the preview.
...
The file is uploaded with strong provenance, to obtain a copy of the [receipt](/learn/receipts), you can request it by passing the transaction id to our SDK function [`bundlr.utils.getReceipt()`](/developer-docs/sdk/api/utils-getReceipt)
