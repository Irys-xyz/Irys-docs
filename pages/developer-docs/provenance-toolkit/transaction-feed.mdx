...
The transaction feed component queries transactions based on node, token, content type and timestamp and renders the results in an easy-to-view format.
...
const queryGraphQL = async (
	...
	if (contentType ?? (null || token) ?? (null || (from ?? null) !== null || (to ?? null) !== null)) {
		query += "(";
		...
		// Check for token (null or undefined)
		if (token ?? null) {
			query += 'token: "' + token + '", ';
		}
	...
	query += " { edges { node { id token timestamp tags { name value } } } } }";
	try {
