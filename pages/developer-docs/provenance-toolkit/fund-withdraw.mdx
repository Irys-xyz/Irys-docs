...
When uploading data to Bundlr, first connect to and then fund a [node](/overview/nodes). Balances are not shared between nodes, fund the node you intend to use.
...
To get the currently funded balance, first connect to a Bundlr node using the node and token selected by the user:

```ts filename="components/FundWithdraw.tsx"
const bundlr = await getBundlr(selectedNode?.value, selectedToken?.value);
```
