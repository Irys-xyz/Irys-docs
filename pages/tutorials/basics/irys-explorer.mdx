...
As you build the project, you'll be introduced to the [Irys Query package](/querying/query-package), which greatly simplifies querying Irys and Arweave. Instead of having to use [GraphQL](/querying/graphql), you can now search transactions through a single JavaScript class, you interact with it as you would with any other JavaScript code.
...
In this tutorial, first, we’ll build out the complete UI, then go back and add in the query functionality. If you’re only interested in how to integrate the `Query` class, feel free to skip ahead to [“Query Functionality”](/tutorials/irys-explorer#query-functionality).
