...
Before diving into this guide, begin with ["Encrypting onchain data"](/tutorials/encryption/encrypting-onchain-data).
...
<Callout type="info" emoji="ℹ️">
	...
	signatures](https://developer.litprotocol.com/v3/sdk/authentication/session-sigs/intro). This guide focuses solely on
	wallet signatures, as session signatures are currently in development and only available for Ethereum.
</Callout>
...
To use Irys to store data on Arweave, first connect [Irys' mainnet or devnet](/overview/networks). This function uses the same private key from our `.env` file and connects to the Irys Devnet where uploads are stored for 60 days. In a production environment, you would change this to use Irys' [mainnet](/overview/networks) where uploads are permanent.

<Callout type="info" emoji="ℹ️">
	This code is configured to Sepolia to pay for uploads, and while working with the Irys Devnet, you need to fund your
	wallet with [free Sepolia](https://www.alchemy.com/faucets/ethereum-sepolia) tokens. Alternatively, you could use [any
	other Devnet token](/overview/supported-tokens#devnet-tokens) supported by Irys.
</Callout>
