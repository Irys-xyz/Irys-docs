...
In this tutorial, you will create a dynamic NFT using Irys' [mutable references](/mutable-references).
...
This tutorial focuses on creating a SuperMon NFT that could evolve during gameplay. The NFT starts with a basic appearance and can be "upgraded" twice. We will use the [Irys CLI](/cli) to update the metadata, simulating the automatic changes that would occur through player interactions in an actual game.

## Mutable references

[Mutable references](/mutable-references) are a way to simulate "mutability".
