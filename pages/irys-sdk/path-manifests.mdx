...
Upon uploading a manifest, you get a receipt containing a transaction ID. To download transactions in a path manifest, request them from the [Irys gateway](/overview/gateways) using a URL formatted as:
...
For more information, [see our guide on uploading static websites](/tutorials/uploading-static-sites).
...
1.  Uses the [Irys query package](/querying/query-package) to search for the 10 most recent transactions tagged `image/png`.
