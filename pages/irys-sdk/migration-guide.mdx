...
When connecting to a `WebIrys` class, you pass both a reference to a provider and the provider type. The following example is for Ethers 5, we also have a full list of supported provider types see [Irys in the browser](/irys-sdk/irys-in-the-browser).
