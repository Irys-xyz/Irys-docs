...
<Callout type="info" emoji="ℹ️">
	The query package is for searching transaction metadata on Irys and Arweave, and also Arweave block information. Once
	you've found transactions, use the transaction ID to [download](/downloading) the associated data.
</Callout>
...
The search location is specified by passing a parameter to the [`search()`](/query-sdk/api/search) function.

```js
const results = await myQuery.search("irys:transactions");
```

The selected search type influences the [returned fields](/querying/query-package#limiting-fields-returned) and the availability of specific query functions.
...
Use the `tags()` function to search [metadata tags](/irys-sdk/tags) attached to transactions during upload.
