...
Queries return transaction metadata. To then retrieve data, use the returned transaction ID and download the data from a [gateway](/overview/gateways) using a URL formed as follows `https://gateway.irys.xyz/[transaction-id]`.
...
Search for transactions matching [tag](/irys-sdk/tags) name / value pairs:
