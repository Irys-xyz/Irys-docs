...
When you post a transaction to Irys, you are instantly given back a [signed receipt](/learn/receipts) from the [node you upload to](/overview/network). While it takes over 100 minutes to achieve transaction finality on Arweave, we **optimistically** consider the transaction **final** at upload time as our nodes have internal safeguards that ensure transaction finality on Arweave.
