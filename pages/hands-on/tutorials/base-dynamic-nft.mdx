---
description: Learn how to create a dynamic NFT using Irys' mutable references.
---
...
<img
	...
	src="/img/tutorials/dynamic-nft/all-characters.png"
/>
...
- Create NFT metadata and mint as an NFT.
- "Mutate" the NFT metadata using Irys' mutable references.

## Prerequisites

1. **Setup a Coinbase Wallet:** You'll need a web3 wallet; you'll use it to deploy the NFT contract and to sign and pay for uploads to Irys. We recommend using the [Coinbase Wallet](https://chrome.google.com/webstore/detail/coinbase-wallet-extension/hnfanknocfeofbddgcijnmhnfnkdnaad?hl=en).

2. **Wallet Funding:** You'll need to fund your wallet with some Base Sepolia tokens. You can do this for free using the [Base Faucet](https://docs.base.org/tools/network-faucets).
...
Irys has a pay-once-store-forever model and accepts payment for storage using ETH on Base.
...
and can be "upgraded" twice. We will use the Irys CLI to "mutate" the metadata, simulating the automatic changes that would occur through player interactions in an actual game.
...
[Deploy the smart contract using Remix](https://docs.base.org/tutorials/deploy-with-remix/).

## Irys CLI

We'll use the [Irys CLI](https://docs.irys.xyz/developer-docs/cli) to upload images and metadata.
...
When executing CLI commands involving funding and signing, you must provide a private key. Use the `-w` flag to specify a private key along with the -t flag to indicate the token you'll use.
...
Next, fund the Irys Devnet node with 0.1 [Base Sepolia ETH](https://docs.base.org/tools/network-faucets) to pay for your uploads.`
...
> Data on Irys' devnet is kept for ~60 days and are paid for using free tokens available from faucets (Base Sepolia). Uploads to Irys' mainnet are permanent and can be paid for using ETH on Base.
...
<img
	className="border border-[#FEF4EE] rounded-xl mt-5 lg:w-5/6 w-full"
	src="/img/tutorials/dynamic-nft/open-sea-mockup.jpg"
/>
...
<img
	className="border border-[#FEF4EE] rounded-xl mt-5 lg:w-5/6 w-full"
	src="/img/tutorials/dynamic-nft/refresh-metadata.png"
/>
