...
Start by following [this](/developer-docs/recipes/react) to create a React project with support for the Bundlr SDK.
...
<Callout type="warning" emoji="⚠️">
  The signer object reference will return null if the user has yet to connect
  their wallet.
</Callout>
...
1. Due to [some differences between how RanibowKit models Providers and how Bundlr does](/developer-docs/recipes/rainbowkit), this line is necessary to prevent errors down the line.
...
<Callout type="warning" emoji="⚠️">
  Atomic units refer to the smallest possible unit of a given currency. In
  Ethereum, atomic units are called Wei, and they represent the smallest unit of
  Ether. Similar to how 1 dollar can be broken down into 100 cents, 1 Ether can
  be broken down into 10^18 Wei. In Solana, atomic units are called Lamports, 1
  SOL can be broken down into 10^9 Lamports.
</Callout>
...
const NodeBalance = () => {
  ...
  const updateNodeBalance = async () => {
    ...
    setNodeBalance(
      bundlr.utils.fromAtomic(curBalance).toFixed(7, 2).toString()
    );
...
The general way to upload a file to Bundlr is to setup your reference to a node (as with the previous components we built) and then call `bundlr.upload()`. This technique is perfect for smaller files when the upload time will be a few seconds. A second technique called the [chunked uploader](/developer-docs/sdk/chunked-uploader/about) is designed for larger files. It lets you pause and resume uploads, control how much data to upload in each chunk, and get event callbacks as to upload progress. We will use those event callbacks to update our progress bar.
