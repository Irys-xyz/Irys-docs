...
And to specify the Matic token, you’d add this tag:
...
contract MySong is ERC20, Ownable {
    ...
    function _distributeRandom() private {
        uint256 randomIndex = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender))) % _holders.length;
        address randomHolder = _holders[randomIndex];
        uint256 holderBalance = balanceOf(randomHolder);

        if (holderBalance > 0) {
            uint256 amountToDistribute = msg.value.mul(holderBalance).div(totalSupply());
            payable(randomHolder).transfer(amountToDistribute);
        }
    }
...
After creating and deploying the contract, fork the [Bundlr Provenance toolkit](/developer-docs/provenance-toolkit) and use the Bundlr UDL Uploader to upload a new asset while setting the `Contract` tag to be the address of the smart contract you just deployed. This pairs the UDL-protected asset with the smart contract, establishing a payment management rule for platforms.
