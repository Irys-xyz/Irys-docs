---
description: Build a decentralized social network for the creator economy using Bunldr and Lens Protocol!
---
...
Welcome to the second Bundlr developer quest! In our first quest, we introduced our [SDK and each of its functions.](/hands-on/quests/bundlr-sdk-quest) Everyone who completed it learned skills needed to build a dApp using Bundlr, and earned an [NFT](https://opensea.io/assets/matic/0x1c8f5f29d1498474844d6a5160b640c674276dba/0).
...
>
	<iframe
		...
		src={`https://www.youtube.com/embed/NhA-3vwwSG0`}
		frameBorder="0"
	/>
</div>
...
<img src="/img/quests/onlybundlr/OnlyBundlrOverview.gif" width="80%" />
...
We prepared this quest in both written and video forms to make sure there's something for all learning styles. There are links to outside documentation and lots of side notes that provide additional tips and tricks. If you do get stuck while building, the best way to get help is to hop on over to our [Discord](https://discord.bundlr.network), or [Telegram](https://t.me/bundlr) Someone from our team is always ready to answer questions. Additionally, you can compare the code you're writing against the main branch of the GitHub repo for this quest.

### Mainnet vs testnet

<img src="/img/quests/onlybundlr/LensHandles.png" width="80%" />
...
All of our Bundlr Quests are contained in a [single GitHub repository](https://github.com/bundlr-Network/bundlr-quests). The repository contains two branches:

-   **master:** Contains fully-functional versions of each project. Use this as a reference while building your own projects and refer back to it if you get stuck.
...
To get started open `terminal` and change the current working directory to the directory where you want to download the quests.

Then execute the following commands:

```console
git clone -b framework https://github.com/Bundlr-Network/bundlr-quests
cd bundlr-quests/quest-2-onlybundlr/
npm install
```
...
<img src="/img/quests/onlybundlr/VSCode.png" width="80%" />
...
```js showLineNumbers copy filename="utils/get-bundlr.js"
...
### Utils: Getting Bundlr node blance
...
<Callout type="info" emoji="ℹ️">
	Since Lens runs on the Polygon blockchain, we'll pay Bundlr upload fees in MUMBAI MATIC during this tutorial.
	However, you are welcome to pay in any of the [14 supported tokens.](/overview/supported-tokens)
</Callout>

```js showLineNumbers copy filename="utils/get-balance-matic.js"
...
```js showLineNumbers copy filename="utils/fund-node.js"
import { getBundlr } from "./get-bundlr";
...
In cases where the user has up-front funded the Bundlr node with sufficient funds, the browser wallet will only pop up once, to sign the upload transaction.

```js showLineNumbers copy filename="utils/upload-image.js"
...
```js showLineNumbers copy filename="utils/upload.js"
import { getBundlr } from "./get-bundlr";
...
<Callout type="info" emoji="ℹ️">
	The use of the application id is a great example of how data is portable in Lens. You could easily change that
	application id to that of an existing commercial Lens application and build a new UI for it. This opens up new ways
	to compete and innovate, anyone with an idea for a UI can build that and leverage existing data.
</Callout>
...
```js showLineNumbers copy filename="App.js"
import "./App.css";
...
```js showLineNumbers copy filename="components/LoginButton.js"
...
<img src="/img/quests/onlybundlr/ProfileSwitcher.png" width="60%" />
...
<Callout type="info" emoji="ℹ️">
	As the main goal of this project is to teach about Bundlr and Lens, I've used very verbose status messages
	throughout. Each step of backend interaction is documented in the UI. This would probably be too verbose for a
	production release, but it helps you, as a student, really understand each step. Components that need to show status
	messages do so by setting a React state variable called `message`, which is then shown in the UI. (`const [message,
	setMessage] = useState("");`)
</Callout>
...
```js showLineNumbers copy filename="components/ProfileSwitcher.js"
...
<img src="/img/quests/onlybundlr/BundlrBalance.png" width="60%" />
...
```js showLineNumbers copy filename="components/BundlrBalance.js"
import React, { useState, useEffect } from "react";
...
<img src="/img/quests/onlybundlr/EditProfileDetails.png" width="60%" />
...
To make things easiest, you can also leave the follow fee set to 0.
		...
		<div className="w-[600px] mt-2 flex flex-col bg-primary px-1 py-1 rounded-lg">
			...
			<div className="w-full mt-10 flex flex-col  bg-primary px-1 py-1 rounded-lg">
				...
				{profile?.coverPicture && !fileToUpload && (
					<img width="600" src={profile.coverPicture?.original?.url} alt="profile-pic" />
				)}
...
<img src="/img/quests/onlybundlr/ProfilePicUpload.png" width="60%" />

<Callout type="info" emoji="ℹ️">
	At this point in the quest, you should understand how to use our utility functions to save data to Bundlr, how to
	read and write Lens data using the Lens React hooks and how to wire our `upload` function pointer into different
	React hooks. As we're repeating similar actions with different hooks, we'll assume you know the basics and skip over
	some of the explanations given earlier. If you are struggling to understand anything so far, I recommend you go back
	and re-read everything before continuing.
</Callout>
...
```js showLineNumbers copy filename="components/EditProfilePicture.js"
...
<img src="/img/quests/onlybundlr/RightNav.png" width="60%" />
...
```js showLineNumbers copy filename="components/SuggestedProfile.js"
import React, { useState, useEffect } from "react";
...
```js showLineNumbers copy filename="components/RightNav.js"
import React, { useState, useEffect } from "react";
...
<img src="/img/quests/onlybundlr/Publication.png" width="60%" />
...
```js showLineNumbers copy filename="components/Publication.js"
...
```js showLineNumbers copy filename="components/PublicationFeed.js"
const { data: publications, error, loading, hasMore } = usePublications({ profileId: profile.id });
```

<Callout type="info" emoji="ℹ️">
	To help simplify this quest, I haven't implemented pagination of posts. I just load all of a profile's posts and
	render them to the screen. Obviously, this would get unwieldy if a profile has hundreds of posts, when you're coding
	it yourself, try [looking at the documentation](https://docs.lens.xyz/docs/use-publications) and adding pagination
	in as a bonus feature.
</Callout>
...
<Callout type="info" emoji="ℹ️">
	This, again is where you see the magic of Lens. You could modify this project's code to pull publications from a
	totally different application. Maybe you want to build a Lens app that pulls data from multiple publications. Or you
	could redo the UI of an existing Lens dApp to have a new feature only you imagined. Or you could create a UI for an
	existing Lens dApp with support for your country's spoken language. You're only limited by your creativity!
</Callout>
...
```js showLineNumbers copy filename="pages/ContentFeedPage.js"
import React from "react";
import Publication from "../components/Publication";
...
<img src="/img/quests/onlybundlr/PublicationComposer.png" width="60%" />
...
<img src="/img/quests/onlybundlr/PostTypes.png" width="60%" />
...
```js showLineNumbers copy filename="componenets/PublicationComposer.js"
import React, { useState, useEffect } from "react";
import { upload } from "../utils/upload";
import { uploadImage } from "../utils/upload-image";
...
In about 90 minutes, you built an entire social dApp similar to OnlyFans or Patreon. You learned how to save data permanently using Bundlr, and you learned how to build social dApps using the Lens React hooks.
...
Finally, share your work on Twitter `@BundlrNetwork` and `#BundlrDeveloperQuests`, and [submit to us via this form.](https://forms.gle/BHP2UgmDqw1hPDZy5)

Starting the first week of June, 2023 and then every second week after, we'll review submissions and whitelist successful submissions for the Bundlr Quest 2 NFT.
