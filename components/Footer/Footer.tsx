...
const Footer = () => {
	...
	const FooterLink = ({ href, name, newTab }: { href: string; name: string; newTab?: boolean }) => (
			...
			<section className="flex flex-col xl:flex-row items-center justify-between mt-20 gap-20">
				<div>
					{DEFINITIONS.map((definition) => (
						<div key={definition.title} className="flex items-center gap-10 flex-col">
				...
				<p className="flex items-center gap-2 text-[#B7B7B7]">{currentYear} Irys. All rights reserved.</p>
			</section>
		</footer>
	);
};
